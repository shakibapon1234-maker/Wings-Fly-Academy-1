...
const SettingsModule = (() => {
  ...
  function switchTab(tab) {
    ...
    if (tab === 'student-portal') {
      setTimeout(() => {
        if (window.SettingsStudentPortal) {
          window.SettingsStudentPortal.inject();
        } else if (window.LazyModules) {
          window.LazyModules.ensure('settings-student-portal').then(() => {
            window.SettingsStudentPortal && window.SettingsStudentPortal.inject();
          });
        }
      }, 80);
    }
  ...
  function panelData() {
    ...
    <div class="settings-panel ${activeTab === 'data' ? 'active' : ''}" data-panel="data">
      ...
      <div class="settings-card glow-red">
        ...
        <button class="btn btn-outline btn-sm" style="border-color:var(--error);color:var(--error);margin-bottom:10px;width:100%;justify-content:center" onclick="SettingsModule.cleanDuplicateRepairEntries()">
...
<div class="footer">
  ...
  // ════════════════════════════════════════════════════════════════
  // TAB 11: MONITOR — delegates to settings-monitor.js (SettingsMonitor)
  // ════════════════════════════════════════════════════════════════
  function panelMonitor() {
    ...
    return `<div class="settings-panel ${activeTab === 'monitor' ? 'active' : ''}" data-panel="monitor">
      <div class="settings-card glow-purple">
        <div class="settings-card-title"><i class="fa fa-chart-line"></i> DATA MONITOR</div>
        <p style="color:var(--text-muted);font-size:.85rem">Loading monitor module…</p>
      </div>
    </div>`;
  }
  // ════════════════════════════════════════════════════════════════
  ...
  async function cleanDuplicateRepairEntries() {
    ...
    const dups = all.filter(function(f) {
      return (f.description && (f.description.includes('(Repaired)') || f.description.includes('(Auto-healed)'))) || 
             (f.note && f.note.includes('Auto-repaired')) || 
             (f.id && f.id.startsWith('REPAIR-'));
    });
    
    if (dups.length === 0) {
      Utils.toast('✅ কোনো duplicate entry নেই', 'success');
      return;
    }
    
    const ok = await Utils.confirm(dups.length + 'টি duplicate Repair entry মুছবেন? (Balance পরিবর্তন হবে না)', 'Clean Duplicates');
    if (!ok) return;
  ...
  */
  ...
  function panelStudentPortal() {
    return `<div class="settings-panel" data-panel="student-portal" id="settings-student-portal-panel">
  ...
  function panelSMSSettings() {
    ...
    return (typeof window._panelSMSSettings_html === 'function')
      ...
      : `<div data-panel="sms-settings" class="settings-panel">
