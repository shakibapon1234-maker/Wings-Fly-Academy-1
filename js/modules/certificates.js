...
const CertificatesModule = (() => {
  ...
  function _certBaseUrl() {
    return (typeof Utils !== 'undefined' && Utils.resolveAppUrl)
      ? Utils.resolveAppUrl('certificate.html').replace(/certificate\.html.*$/, '')
      : `${window.location.origin}${window.location.pathname.replace(/[^/]*$/, '')}`;
  }

  function _certPageUrl(token) {
    const base = (typeof Utils !== 'undefined' && Utils.resolveAppUrl)
      ? Utils.resolveAppUrl('certificate.html')
      : `${_certBaseUrl()}certificate.html`;
  ...
  function buildCertHTML(data) {
    ...
    <div class="cert-v2-container">
      ...
      <div class="cert-signature" style="bottom:3%;left:10%;right:auto;text-align:center;">
        <img src="${sigShakib}" onload="typeof CertificatesModule !== 'undefined' && CertificatesModule.processSignature && CertificatesModule.processSignature(this, true)" style="height:50px;margin-bottom:5px;opacity:0.9;" onerror="this.style.display='none'">
      ...
      <div class="cert-signature" style="bottom:3%;right:10%;left:auto;text-align:center;">
        <img src="${sigChairman}" onload="typeof CertificatesModule !== 'undefined' && CertificatesModule.processSignature && CertificatesModule.processSignature(this, false)" style="height:50px;margin-bottom:5px;opacity:0.9;" onerror="this.style.display='none'">
  ...
  function processSignature(imgElement, invertText) {
    ...
    setTimeout(() => {
      ...
      try {
        ...
        console.error("Signature transparency processing failed: ", e);
      }
    }, 50);
  }
  ...
  function print(data) {
    ...
    <script>
      const CertificatesModule = {
        processSignature: ${processSignature.toString()}
      };
  ...
  async function showQRModal(studentDbId) {
    ...
    const certUrl = _certPageUrl(token);
  ...
  async function generateAllQRCards() {
    ...
    <script>window.addEventListener('load',function(){setTimeout(function(){window.print();},1800);});}<` + `/script></body></html>`);
    win.document.close();
  }
      ...
      <div style="background:linear-gradient(135deg,#eff6ff,#dbeafe);border:1px solid #93c5fd;border-radius:10px;padding:12px 16px;margin-bottom:20px;display:flex;align-items:center;gap:12px;flex-wrap:wrap;">
        ...
        <div style="display:flex;align-items:center;gap:8px;background:#fff;padding:4px 4px 4px 12px;border-radius:24px;border:1px solid #bfdbfe;">
          <div class="blur-link-container" style="position:relative;cursor:pointer;overflow:hidden;min-width:180px;" onclick="this.classList.toggle('revealed')">
            <span class="blur-text" style="filter:blur(4px);transition:filter 0.3s;font-family:monospace;font-size:0.85rem;color:#1e3a8a;">${Utils.esc(portalLink)}</span>
          ...
          <button class="btn-primary btn-sm" onclick="Utils.toast('Link Copied!','success');navigator.clipboard.writeText('${Utils.escAttr(portalLink)}')" style="border-radius:20px;white-space:nowrap;padding:4px 12px;"><i class="fa fa-copy"></i> Copy portal link</button>
        </div>
        <style>
