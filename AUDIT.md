...
| Date | Change |
|------|--------|
| 2026-07-01 | **Finance Balance Architecture Fix (v2):** `supabase-sync.js` এ `repairMissingStudentFinance()` কে Ledger-only repair-এ রূপান্তরিত করা হয়েছে — এটা এখন আর `updateAccountBalance()` call করে না (incremental counter corrupt হওয়া রোধ)। নতুন `recalculateAccountBalancesFromLedger()` function যোগ হয়েছে যেটা সম্পূর্ণ Finance Ledger থেকে sum করে account balance fresh calculate করে (zero-drift approach)। `backup-restore.js` এ import-এর পরে automatically এই recalculation চালানো হয় এবং `wfa_finance_backfill_v1` flag set হয় (repair আর auto-run করবে না)। Settings → Data Management এ নতুন **"🧮 Recalculate Balance from Ledger"** বাটন যোগ হয়েছে। **মূল কারণ ছিল:** `wfa_finance_backfill_v1` localStorage flag DB switch/cache purge-এ মুছে যায়, ফলে startup repair বারবার চলত এবং incremental balance update double-count করত। Affected files: `js/core/supabase-sync.js`, `js/core/backup-restore.js`, `js/ui/settings.js` |
| 2026-07-01 | **Loan & Investment Balance Fix:** `recalculateAccountBalancesFromLedger()` এ bug fix — আগে `_isLoan: true` flag দিয়ে সব Loan entry skip করা হতো, কিন্তু Loan Giving (cash বের হয়) ও Loan Receiving (cash আসে) সত্যিকারের cash flow। এখন diagnostic loan notes ছাড়া সব Loan Giving/Receiving ও Investment In/Out balance-এ count হয়। Recycle bin restore logic-এ (`supabase-sync.js` L1616) Investment In/Out type check missing ছিল — restore করলে balance আপডেট হতো না। দুটোই fix হয়েছে। Affected file: `js/core/supabase-sync.js` |
