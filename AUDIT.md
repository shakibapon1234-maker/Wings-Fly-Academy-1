...
| Date | Change |
|------|--------|
| 2026-06-25 | **Feature 3 — Class Routine Builder (v5.0.0):** New `routine-engine.js` (core CRUD + conflict detection) and `routine-builder.js` (admin UI — weekly grid/list view, teacher conflict warning, print/PDF export); `class_routines` table added to `supabase-sync.js` TABLE_COLUMNS (both sync + backup sections); `lazy-modules.js` registered `routine-engine` + `routine-builder` with dep chain; `index.html` nav + section + mobile more-menu updated; `app.js` SECTIONS array, title map, permissions map, heavyModules map, and renderModule switch updated; SW `DEPLOY_ID` bumped to `20260625-feature3-routine-builder` |
