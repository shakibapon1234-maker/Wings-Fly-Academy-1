...
| Date | Change |
|------|--------|
| 2026-07-01 | **Finance Repair Balance Bug Fix:** `supabase-sync.js` `repairMissingStudentFinance()` এ দুটো bug fix করা হয়েছে। **Bug #1:** পুরনো `REPAIR-*` entry গুলো `remove()` দিয়ে মুছলে account balance reverse হতো না — এখন প্রতিটি Income/Expense entry সরানোর আগে `updateAccountBalance()` দিয়ে effect উল্টিয়ে দেওয়া হয়। **Bug #2:** নতুন repair entry `insert()` করার পর `updateAccountBalance('in')` কল হতো না, ফলে Finance Ledger-এ entry দেখা গেলেও account balance বাড়তো না বা double count হতো। এখন `insert()` এর পরেই explicit `updateAccountBalance(defaultMethod, unrecorded, 'in')` call যোগ করা হয়েছে। **মূল কারণ:** `wfa_finance_backfill_v1` localStorage flag টি DB switch বা cache purge-এ মুছে যাওয়ায় startup repair বারবার চলছিল। Affected file: `js/core/supabase-sync.js` (L2663–2680, L2759). Repair button location: **Settings → Data Management → Finance Ledger Repair → "Repair Missing Finance Entries"** |
