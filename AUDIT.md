...
| Date | Change |
|------|--------|
| 2026-07-01 | **Balance Corruption Root Cause Fix (v3 — Final):** তিনটি AI অডিটে প্রমাণিত মূল কারণগুলো fix করা হয়েছে। **(1) Auto-startup repair disabled:** `_runStartupFinanceRepair()` এ `repairMissingStudentFinance()` call সম্পূর্ণ বন্ধ — শুধু localStorage flag set হয়। localStorage flag মুছে গেলেও repair আর চলবে না। **(2) Backup import-এ auto-recalculate removed:** `backup-restore.js`-এ `recalculateAccountBalancesFromLedger()` auto-call সরানো হয়েছে — backup-এ duplicate entries থাকলে এটা balance ৫০x বাড়িয়ে দিত। **(3) Recalculate button replaced:** Settings-এর "Recalculate Balance from Ledger" (AUDIT_IGNORE Section 8 violation) সরিয়ে "🧹 Clean Duplicate Entries" button দেওয়া হয়েছে — এটা শুধু duplicate REPAIR entries মুছে, balance touch করে না। **(4) SyntaxError Fix:** বাটনের inline onclick-এ backtick template literal interpolation (${}) ও arrow function ব্যবহারের কারণে `SyntaxError` আসছিল (settings.js layout template interpolation-এর সাথে সাংঘর্ষিক হওয়ায়)। এটি ফিক্স করতে logical function-টি `SettingsModule.cleanDuplicateRepairEntries` হিসেবে মডিউল স্কোপে ডিফাইন করে UI থেকে কল করা হয়েছে। **Root cause confirmed:** 103টা duplicate `(Repaired)` entry finance_ledger-এ জমেছিল (৳8,10,497 phantom amount) — বারবার startup repair চালানোর ফলে। Affected files: `js/core/supabase-sync.js`, `js/core/backup-restore.js`, `js/ui/settings.js` |
