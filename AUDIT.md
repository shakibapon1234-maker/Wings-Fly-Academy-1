...
| Date | Change |
|------|--------|
| 2026-07-11 | **License Server Admin Secret Overwrite Fix & File Sync:** (1) Simplified storage logic to read directly from localStorage (no auto-saves from DOM) and changed input type to password/autocomplete=new-password to prevent browser autofill corruption. (2) Synced downloaded `institution-mode.js` and `settings-institution.js` across root, www, and android folders. Affected: `js/core/license-manager.js`, `js/ui/settings.js`, `js/core/institution-mode.js`, `js/core/settings-institution.js`. |
| 2026-07-10 | **Monitor Snapshot Integrity:** Removed both Rebuild Data controls and public entry points. Data Monitor now shows only genuine stored transaction snapshots; it cannot manufacture misleading historical balances. SW DEPLOY_ID → 20260710-monitor-real-snapshots. |
| 2026-07-10 | **Activity Log + Data Monitor Audit:** Activity Log cloud merge now re-pushes offline-created local entries and keeps same-timestamp rows in a stable newest-first order. Data Monitor shows stored snapshots as previous recorded balance to current recorded balance, without calculating or writing balances. Affected: `js/core/supabase-sync.js`, `js/ui/activity-log.js`, `js/ui/settings-monitor.js`; SW `DEPLOY_ID` → `20260710-activity-monitor-audit`. |
